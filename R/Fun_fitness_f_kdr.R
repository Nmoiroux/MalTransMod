...
#' @param fi1_u_kdr a vector of size three of which elements are the Successful feeding probability of alive mosquitoes in a hut with an unprotected individual (genotype RR, RS and SS respectively)
#' @param RR_fi1_kdr a vector of size three of which elements are risk ratio of successful feeding for alive mosquitoes in a hut with LLIN (compared to hut without LLIN)
...
#' fi1_u_kdr <- c(0.55,0.55,0.55)			# default values
#' RR_fi1_kdr <- c(0.56,0.56,0.56)    # default values
#' fitness_f_kdr(Pllin_kdr, m1p_kdr, m2p_kdr, success_not, success_net, biting_not, biting_net, VLAIB)

fitness_f_kdr <- function(Pllin_kdr, m1p_kdr, m2p_kdr, fi1_u_kdr, RR_fi1_kdr, FUN=VLAIB){
